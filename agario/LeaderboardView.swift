...
class LeaderboardView: UIView {
    ...
    func setLeaderboardContent(_ l : [Dictionary<String, Any>]) {
        for i in lbContent {
            i.text = ""
        }
        
        for i in 0 ..< min(l.count, 5) {
            let name = String(describing: l[i]["name"]!)
            let score = String(describing: l[i]["score"]!)
    ...
    func setup() {
        ...
        lbGroup.backgroundColor = UIColor.white
        ...
        exitBtn.setImage(UIImage(named: "icon-exit"), for: UIControlState())
        exitBtn.frame.origin.x = lbGroup.frame.width - exitBtn.frame.width
        exitBtn.bounds = exitBtn.frame.insetBy(dx: 5.0, dy: 5.0)
