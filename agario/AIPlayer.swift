...
class AIPlayer : Player {
	...
	override init(playerName name : String, playerValue value: Int, parentNode parent : SKNode, initPosition p : CGPoint) {
		super.init(playerName: name, playerValue: value, parentNode: parent, initPosition: p)
        ...
        if let b = self.children.first as! Ball? {
            if b.physicsBody?.velocity == CGVector(dx: 0, dy: 0) {
                ...
                for food in scene.foodLayer.children as! [Food] {
					if (distance(food.position, p2: self.centerPosition()) < b.radius * 10) && (food.sufValue == b.ballValue) {
